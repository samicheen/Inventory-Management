...
export class ItemSalesReportComponent implements OnInit {
  ...
  initializeColumns(): void {
    this.columns = [
      ...
      { key: 'total_customers', label: 'Total Customers', sortable: true, valueFormatter: (val) => val || 0 },
      { 
        key: 'total_quantity', 
  ...
  loadReport(): void {
    ...
    this.reportsService.getItemWiseSales(startDate, endDate).subscribe({
      next: (response) => {
  ...
  exportToExcel(): void {
    ...
    const data = this.items.map(item => ({
      ...
      'Total Revenue': parseFloat(item.total_amount || 0).toFixed(2),
      'Avg. Selling Price': parseFloat(item.average_selling_price || 0).toFixed(2),
      'First Sale Date': item.first_sale_date ? this.formatDate(item.first_sale_date) : '-',
      'Last Sale Date': item.last_sale_date ? this.formatDate(item.last_sale_date) : '-'
    }));
    ...
    data.push({
      ...
      'Total Revenue': parseFloat(this.summary.total_amount || 0).toFixed(2),
      'Avg. Selling Price': '-',
      'First Sale Date': '-',
      'Last Sale Date': '-'
    });
    ...
    const fileName = `Item_Sales_Report_${new Date().toISOString().split('T')[0]}.xlsx`;
